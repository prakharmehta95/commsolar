...
def community_combinations(data, distances, df_solar, df_demand, df_solar_combos_main , df_demand_combos_main, Combos_formed_Info):
    ...
    data['adoption'] = ""
    data['adoption'] =  [ 1 if i%2 == 1 else 1 for i in range(len(data.index))] # temporarily setting adoption so that I can replicate what happens in the ABM
    
    data['community'] = ""
    data['community'] =  [ 1 if i%2 == 1 else 0 for i in range(len(data.index))] # temporarily setting community adoption so that I can replicate what happens in the ABM
    data.at['B147891','community'] = 0
    #data.at['B147890','community'] = 1
    data.at['B147890','community'] = 1
    data.at['B147893','community'] = 0
    ...
    for i in ['B147891']: #data.index: THIS IS THE BUILDING BEING CONSIDERED IN THE ABM VIA THE self.uid...
        if data.loc[i]['intention'] == 1: #find other buildings in same plot with positive intention
            ...
            same_plot_agents_positive_intention = same_plot_agents[same_plot_agents['intention'] == 1] #available to form community
    ...
    if len(same_plot_agents_positive_intention.index) > no_closest_neighbors_consider:
        ...
        combos_consider = same_plot_agents_positive_intention.head(5) #closest 4 neighbours
    ...
    temp_combos_list_filter = [combos_consider.loc[i]['Comm_formed'] if combos_consider.loc[i]['community'] == 1 else i for i in temp_combos_list_temp]
    #temp_combos_list_filter = temp_combos_list_temp
    #%make the possible combinations    
    
    import itertools
    temp_combos_list = []
    constant =  'B147891' #read this from the self.uid
    temp_combos_list_filter.remove('B147891') #so that the energy_champion building does not get eliminated in the combos - the combos are made without it and then this building is added to all the combos
    ...
    for i in range(len(temp_combos_list)):
        ...
        #for the consumer case of the 2 agent situation
        temp_solar_2 = 0
        temp_demand_2 = 0
        temp_name_2 = ""
        temp_pv_list_2 = []
        ...
        for j in range(len(temp_combos_list[i])):
            ...
            try:
                ...
                #keyError will only occur in case an existing community is one of the choices for the agent, hence read info from the combos dataframes
                print("KeyError")
                ...
                #maybe use different variables so that they are accessible
                    ...
                    temp_demand_2 = temp_demand #+ df_demand_combos_main[temp_bldg] #temp_demand must be considered as the KeyError only happens with the second agent; I have hardcoded it this way
                    temp_pv_size_2 = temp_pv_size #+ Combos_formed_Info.loc[temp_bldg]['combos_pv_size_kw']
                    temp_num_smart_meters_2 = temp_num_smart_meters #+ Combos_formed_Info.loc[temp_bldg]['combos_num_smart_meters']
                    temp_num_members_2 = len(temp_combos_list[i])#temp_num_members + combos_consider.loc[temp_bldg][''] if this is wromg, hardcode to 2
